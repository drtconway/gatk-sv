process GATK4_SVCLUSTER {
    ...
    stub:
    // Patched locally: upstream's stub only touches *.vcf.gz, but the
    // declared `output:` above (clustered_vcf_index) requires *.vcf.gz.tbi
    // too, so -stub-run fails with "Missing output file(s)". Diverges from
    // github.com/nf-core/modules intentionally and narrowly -- doesn't
    // touch `script:`, only the stub used for wiring validation.
    def prefix = task.ext.prefix ?: "${meta.id}"
    """
